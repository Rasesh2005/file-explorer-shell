...
1. `getLastSyncTime()` reads the last indexed timestamp from `db_files[ino=0]` (a sentinel metadata entry).
2. During traversal, each file's `mtime` is compared to its stored `mtime`.
3. Only **new or modified files** are re-indexed.
4. `setLastSyncTime(now)` updates the sentinel.
